        ...
        <ul class="others">
          <li v-for="hotRead in hotReadList" :key="hotRead.id">
              ...
              <p class="info">
                ...
                <span class="user" v-if=!isUserHomeFlag><a @click="selectMenu(hotRead.userName)"><iv-icon type="ios-contact"></iv-icon> {{hotRead.userName}} </a></span>
                <span class="readings" v-if=isUserHomeFlag><a ><iv-icon type="ios-eye-outline" color="#76EE00"></iv-icon> {{hotRead.readCount}} </a></span>
              </p>
          </li>
        </ul>
      </div>
