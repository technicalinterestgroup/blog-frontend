        ...
        <ul class="others">
          <li v-for="recommend in recommendList" :key="recommend.id">
              ...
              <p class="info">
                ...
                <span class="user" v-if=!isUserHomeFlag><a @click="selectMenu('/' +recommend.userName)"><iv-icon type="ios-contact"></iv-icon> {{recommend.userName}} </a></span>
                <span class="likes" v-if=isUserHomeFlag><a ><iv-icon type="ios-thumbs-up" color="#F78F8F"></iv-icon> {{recommend.likeCount}} </a></span>
              </p>
          </li>
        </ul>
      </div>
